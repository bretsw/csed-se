...
csed <- csed_ques_top %>% 
        inner_join(csed_og[[1]], by = "question_id") %>%
        as.data.frame %>%
        mutate(askers = owner_display_name.x %>% as.character,
               responders = owner_display_name.y %>% as.character
               )

csed$question_id %>% unique %>% length  # these are the number of distinct questions
csed$askers %>% unique %>% length  # these are the different question askers
csed$answer_id %>% unique %>% length  # these are the number of different answers
csed$responders %>% unique %>% length  # these are the different responders
c(csed$askers, csed$responders) %>% unique %>% length # these are total, distinct contributors (askers & responders)
